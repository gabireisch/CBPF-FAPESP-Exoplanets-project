...
def get_positive_train_paths(): return list(Path('dados/train/positives').glob('*.fits'))
def get_negative_train_paths(): return list(Path('dados/train/negatives').glob('*.fits'))
def get_positive_validation_paths(): return list(Path('dados/validation/positives').glob('*.fits'))
def get_negative_validation_paths(): return list(Path('dados/validation/negatives').glob('*.fits'))
def get_positive_test_paths(): return list(Path('dados/test/positives').glob('*.fits'))
def get_negative_test_paths(): return list(Path('dados/test/negatives').glob('*.fits'))

def load_times_and_fluxes_from_path(path):
    try:
        # CONSERTO 1: Converter o Path para string evita erros silenciosos no lightkurve
        lc = read(str(path))
...
def positive_label_function(path): return 1
def negative_label_function(path): return 0

# CONSERTO 2: Uma transformação customizada que faz exatamente o que o qusi faria,
# mas sem conflitos de versão e já retornando os Tensores PyTorch corretos.
def custom_transform(observation, length=2000, randomize=False):
    try:
        ...
        # 4. Normalização (Modified Z-score)
        ...
        # 5. Converte para Tensores PyTorch (o modelo Hadryss exige shape [length, 1])
...
def get_transit_train_dataset():
    positive_train_collection = LightCurveObservationCollection.new(
    ...
    negative_train_collection = LightCurveObservationCollection.new(
        get_paths_function=get_negative_train_paths,
        load_times_and_fluxes_from_path_function=load_times_and_fluxes_from_path,
        load_label_from_path_function=negative_label_function
    )
    train_dataset = LightCurveDataset.new(
        standard_light_curve_collections=[positive_train_collection, negative_train_collection],
        post_injection_transform=train_transform
    )
    return train_dataset

def get_transit_validation_dataset():
    positive_validation_collection = LightCurveObservationCollection.new(
        get_paths_function=get_positive_validation_paths,
        load_times_and_fluxes_from_path_function=load_times_and_fluxes_from_path,
        load_label_from_path_function=positive_label_function
    )
    negative_validation_collection = LightCurveObservationCollection.new(
        get_paths_function=get_negative_validation_paths,
        load_times_and_fluxes_from_path_function=load_times_and_fluxes_from_path,
        load_label_from_path_function=negative_label_function
    )
    validation_dataset = LightCurveDataset.new(
        standard_light_curve_collections=[positive_validation_collection, negative_validation_collection],
        post_injection_transform=val_test_transform
    )
    return validation_dataset

def get_transit_test_dataset():
    positive_test_collection = LightCurveObservationCollection.new(
        get_paths_function=get_positive_test_paths,
        load_times_and_fluxes_from_path_function=load_times_and_fluxes_from_path,
        load_label_from_path_function=positive_label_function
    )
    negative_test_collection = LightCurveObservationCollection.new(
        get_paths_function=get_negative_test_paths,
        load_times_and_fluxes_from_path_function=load_times_and_fluxes_from_path,
        load_label_from_path_function=negative_label_function
    )
    test_dataset = LightCurveDataset.new(
        standard_light_curve_collections=[positive_test_collection, negative_test_collection],
        post_injection_transform=val_test_transform
    )
    return test_dataset